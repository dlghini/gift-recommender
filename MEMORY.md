...
- **Follow-up**: see roadmap item #27 and Phase 16 below — pursued the real affiliate programs (MasterClass, Skillshare, Outschool) to bring virtual-experience recommendations back properly; mixed results, only Outschool actually went anywhere.

### Phase 16: Virtual-Experience Affiliate Program Setup (Awin/Impact) — MIXED RESULTS
...
- **MasterClass**: parked, unresolved. Third-party affiliate-directory sites contradict each other on which network hosts it (Impact / ShareASale / FlexOffers all separately claimed), the ShareASale merchant link for it is dead (redirects to Awin's generic homepage — ShareASale is now part of Awin), and masterclass.com has no discoverable official affiliate page (`/affiliates` 404s). Did not guess-sign-up for a network based on unreliable SEO content-mill sources.
...
27. **Virtual-experience affiliate programs (MasterClass, Skillshare, Outschool)** — researched 2026-08-26 after a bug where Claude suggested a purely virtual "online kids fashion design class" for `store: "viator"`; Viator has no real inventory for online-only experiences, so it silently fell back to a generic sitewide search with irrelevant, multi-country results. Immediate fix shipped (see Phase 15 below) restricts Viator to in-person-only experiences. This item is the follow-up, with mixed results after actually pursuing it:
    - **Outschool** — ✅ application submitted via Awin (2026-08-26), status **Pending**. Awin account fully set up: business profile completed, website channel added and verified (via `impact-site-verification`-style meta tag — see Phase 16 below for the Impact one; Awin's own website channel used its own "Skip verification for now" option, no tag needed).
    - **MasterClass** — **unresolved, parked.** Third-party "affiliate directory" sites disagree on which network hosts it (Impact, ShareASale, FlexOffers all claimed by different sites), the ShareASale merchant link is dead (redirects to Awin's generic homepage), and masterclass.com itself has no discoverable affiliate/partner page (`/affiliates` 404s). Did not sign up for any network on an unverified guess. Revisit only if a reliable, official source turns up.
    ...
    - **Impact.com account** — created and fully set up (business category, website channel `thegiftwhisperer.gifts` verified via meta tag — see Phase 16), but the account's **general Marketplace access was declined** ("You currently do not qualify for access to impact.com's Marketplace... does not affect existing/pending brand program relationships... can still partner directly via brands' unique sign-up links"). So Impact can still be used if a specific brand's own direct sign-up link is found — just can't browse/search Impact's marketplace to discover one.
